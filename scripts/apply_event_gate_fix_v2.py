...
def find_optional_dotenv(root: Path) -> Path | None:
    candidates = (
        root / ".env",
        root / "03_ai-server" / ".env",
        root / "03_ai-server" / "visionflow-ai" / ".env",
    )
...
def apply(root: Path) -> None:
    ...
    required_files = [config, pipeline, main]

    missing = [str(p) for p in required_files if not p.is_file()]
    if missing:
        raise RuntimeError("필수 코드 파일이 없습니다:\n" + "\n".join(missing))

    dotenv = find_optional_dotenv(root)
    backup_files = list(required_files)
    if dotenv is not None:
        backup_files.append(dotenv)

    backup = make_backup(root, backup_files)
    ...
    try:
        ...
        if dotenv is not None:
            ...
            print(f"설정 완료: {dotenv}")
        else:
            print(
                "안내: .env 파일이 없어 환경변수 추가는 건너뜁니다. "
                "config.py 기본값 5프레임/10초가 적용됩니다."
            )
