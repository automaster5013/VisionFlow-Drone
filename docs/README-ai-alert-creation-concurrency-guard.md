# VisionFlow AI 경보 생성 동시성 방어

기준 커밋: `b2c29d76f5c5a082012845b04d9606f467841291`

동일한 AI 추론 이벤트에서 경보 생성 요청이 겹칠 때 두 요청이 모두 `event_id`
멱등 조회를 통과한 뒤 같은 경보를 삽입해 한 요청이 DB UNIQUE 예외로 끝나거나
Incident·실시간 알림 부수 효과를 중복 실행하지 않도록 생성 경로를 직렬화한다.

## 방어 범위

- AI 경보 생성은 대상 AI Inference Event를 `PESSIMISTIC_WRITE`로 잠근다.
- 이벤트 잠금 획득 뒤 `event_id` 멱등 조회를 수행하고, 기존 경보가 없을 때만
  위험도를 계산해 경보를 저장한다.
- 새 경보가 저장된 경우에만 Incident 연계와 생성 실시간 알림을 실행한다.
- 동일 이벤트의 후속 요청은 앞선 트랜잭션 완료 뒤 기존 경보를 확인하고 종료한다.
- V10의 `uk_ai_alert_event` UNIQUE 제약은 중복 경보 생성의 최종 방어선으로
  유지한다.
- 경보 목록·상세와 AI 이벤트 조회는 기존 비잠금 읽기를 유지한다.
...
- AI 이벤트 수집·탐지 저장·경보 위험도·Incident 생성·실시간 발행 규칙은
  변경하지 않는다.
...
```text
[PASS] ai-alert-creation-concurrency-policy
