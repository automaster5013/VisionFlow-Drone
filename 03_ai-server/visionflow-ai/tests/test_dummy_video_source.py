from __future__ import annotations

from pathlib import Path

import cv2
import numpy as np

from app.sources import DummyVideoSource


def _create_test_video(path: Path) -> None:
    writer = cv2.VideoWriter(
        str(path),
        cv2.VideoWriter_fourcc(*"MJPG"),
        10.0,
        (32, 24),
    )
    assert writer.isOpened()

    writer.write(np.zeros((24, 32, 3), dtype=np.uint8))
    writer.write(np.full((24, 32, 3), 255, dtype=np.uint8))
    writer.release()


def test_dummy_video_source_stops_without_loop(tmp_path: Path) -> None:
    video_path = tmp_path / "two-frames.avi"
    _create_test_video(video_path)

    source = DummyVideoSource(
        path=video_path,
        source_id="test-camera",
        session_id="test-session",
        drone_id=1,
        loop=False,
        realtime=False,
    )

    with source:
        assert source.read() is not None
        assert source.read() is not None
        assert source.read() is None


def test_dummy_video_source_restarts_when_looping(tmp_path: Path) -> None:
    video_path = tmp_path / "two-frames.avi"
    _create_test_video(video_path)

    source = DummyVideoSource(
        path=video_path,
        source_id="test-camera",
        session_id="test-session",
        drone_id=1,
        loop=True,
        realtime=False,
    )

    with source:
        packets = [source.read() for _ in range(5)]

    assert all(packet is not None for packet in packets)
    assert [packet.frame_index for packet in packets if packet is not None] == [
        0,
        1,
        2,
        3,
        4,
    ]
